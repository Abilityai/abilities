...
- **Never** "fire a background *shell* task and end the turn to await a notification" in a headless execution — the job is killed the instant the turn ends (only background subagents are waited for, v0.9.0+).
...
**If the deployed agent needs MCP servers of its own**, commit a **`.mcp.json.template`** to the repo — it is the one MCP file that belongs in git (`.mcp.json` itself stays ignored). Since trinity#2007 the container renders it into `~/.mcp.json` at every startup, substituting `${VAR}` / `${VAR:-default}` from the agent's `.env` (inject it in Step 5e). Four rules the platform enforces:
...
- **An `http`/`sse` server's `url` must resolve to a public address.** Loopback, private (RFC 1918), link-local, reserved and **CGNAT `100.64.0.0/10` — the Tailscale range** — are refused with a 400 and no override (trinity-enterprise#394, v0.9.0; enforced by the backend now and by the container after a base-image rebuild). A hostname that resolves there is refused too, DNS failure fails closed. Reach a private MCP server through a public endpoint or run it as a `stdio` server inside the container.
