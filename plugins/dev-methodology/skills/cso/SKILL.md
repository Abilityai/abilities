...
metadata:
  ...
  author: Ability.ai
  changelog:
    - "1.0: Initial version — security audit in CSO mode with --diff (branch changes) and --comprehensive (full codebase) scopes covering secrets, dependencies, auth boundaries, and injection vectors."
---

# CSO Security Audit

> ℹ️ **First, set expectations:** before anything else, print one short line with this skill's version and its most recent change — the top entry of `metadata.changelog` above — e.g. `cso vX.Y — recent: <summary>`. Then proceed.
