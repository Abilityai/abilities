...
- **Don't try to babysit it in-turn.** Past ~10 min the harness **auto-backgrounds** the job (not your choice), active waiting is **blocked** (`sleep`/poll loops rejected — "use monitor with an until-loop"), and the moment the turn ends, **every background Bash job and monitor spawned in it is killed** (the completion event fires as `killed`, not `completed`; the promised re-invoke never comes). Since Trinity v0.9.0 (trinity#2127, rebuilt base image) a headless run **does wait for background subagents/forks** — up to the execution timeout, finalizing after 300s of stream idleness (`AGENT_IDLE_FINALIZE_S`) — but that changes nothing for shell jobs, and the ~10-min Bash ceiling stands. Streaming heartbeat output changes nothing — the no-output stall watchdog (`AGENT_TOOL_STALL_LIMIT_S`, default 1800s) watches `mcp__*` tools only since trinity#1369, Bash isn't watched, and neither the ~10-min ceiling nor the turn-end reaping cares about output. The async monitor / re-invoke model works **interactively** (the session persists) but **NOT** in a headless run — and inside a deployed Trinity agent it does not exist at all: `Monitor`, `TaskOutput`, `ScheduleWakeup`, `CronCreate`/`CronList`/`CronDelete`, `Workflow`, `SendMessage`, `ListAgents`, `PushNotification` and `RemoteTrigger` are platform-denied (trinity#2468/#2454). The platform also stamps every execution's `turn_integrity` (trinity#2467): a `success` row whose response starts `> ⚠️ Background work lost: N background task(s) …` means the turn ended with work discarded — treat it as a failure, not a pass.
...
- **Always verify the artifact moved** (mtime advanced *and* a stats count > 0) before declaring success — never trust the exit code or `business_status`. A run that ends without the artifact changing is a **failure**, not a `skipped`. (A run cut off short of its cap now records `Task execution aborted after Ns of M seconds allowed (…)` with error code `NETWORK` rather than `timed out` — trinity#2752 — so grep for both when reading run history.)

**The Foreground-Fork Rule (headless forks)**: `context: fork` skills run in the **background by default**. A headless/scheduled run is one agent turn; on agents whose base image predates Trinity v0.9.0 (trinity#2127) turn-end reaped every background fork, so a scheduled playbook that invoked a background-forked skill silently lost it — and on v0.9.0+ the wait is bounded by the execution timeout and a 300s idle-finalize, so a slow fork can still be cut. Any skill that uses `context: fork` and is bound for scheduled/headless use — or is composed by a playbook that is — sets `background: false` (wait for the fork's result in-turn, deterministic on every image), or does not fork at all.
