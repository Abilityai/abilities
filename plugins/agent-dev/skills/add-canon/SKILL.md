...
metadata:
  ...
  author: Ability.ai
  changelog:
    - "1.7: Shared projects in canon (operator ruling R21, 2026-09-10 — one project-management standard, two visibility levels; ent#585): a project is managed the same way whether it lives at agent level (project_files/<slug>/ in the managing agent's repo) or at canon level (agents/<owner>/projects/<slug>/) — same project.md charter, same registry epic, same steward, same intake, same decision ledger; canon placement decides visibility only, and company projects are shared by design and live in canon. CONVENTIONS.md template: the folder schema gains projects/ and a § Projects section defining the linted charter envelope (owner = folder, status in the epic's vocabulary, epic owner/repo#N, updated, review_by, tldr) + the append-only decisions.md ledger envelope (never re-stamped); everything else in a slug folder is unlinted workspace. canon-consume 1.4 gains projects mode (<agent> projects [slug] — list, or charter + ledger at a cited sha, staleness per review_by); canon-reconcile 1.4 verifies a stale charter against its epic and re-stamps it, never touches the ledger; canon-publish 1.5 stamps charter/ledger by their own envelopes. Enforcement is add-canon-lint 1.1's project-envelope rule; creation is /project-init --canon (ent#588). Tandem — agents/corbin/projects/tandem/ — is the first shared project and the reference instance"
...
| **Shared projects** | the shared canon repo, `agents/<name>/projects/<slug>/` | company projects this agent manages — the same `project.md` charter + `decisions.md` ledger as an agent-level `project_files/<slug>/`; in canon so every agent and human can read them (ruling R21) |
