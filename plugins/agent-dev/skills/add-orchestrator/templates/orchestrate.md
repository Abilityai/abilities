...
metadata:
  ...
  author: orchestrator
  changelog:
    - "1.14: Drop the three vestigial schedule tools (create_agent_schedule/delete_agent_schedule/list_agent_schedules) from allowed-tools — zero positive body usage remains since the v1.7 watchdog moved fully to set_reminder/cancel_reminder (the only surviving mention is a negative one: 'no schedule litter, no delete_agent_schedule'). Least-privilege trim; no behaviour change"
    - "1.13: Dispatch is a playbook call (fleet convention protocols/playbook-call.md, operator direction 2026-08-16) — Step 3 resolves each step to a playbook from the target's LIVE catalog (get_agent_skills, now in allowed-tools; never the map/profile snapshot) and the dispatched message is one line `/<playbook> [args] --run <task_id>`; a prose brief is the exception, recorded as freeform: true on the ledger entry and reported as a fleet gap"
...
- **Resolve the work to a playbook, not a brief** (fleet convention `protocols/playbook-call.md`): read the target's *live* catalog with `mcp__trinity__get_agent_skills` (never the map or a fleet-profile snapshot — playbooks change faster than profiles) and pick the playbook that owns this step. The dispatched message is then **one line**: `/<playbook> [declared args] --run <task_id>` — no restated policy, no preamble; the playbook is the contract. If no playbook fits, dispatch a prose brief **and record `freeform: true`** on the ledger entry — that is a fleet gap to report (the owner should grow a playbook), not the normal path.
...
d. **Dispatch and record:** `chat_with_agent(<deployed_name>, prompt, parallel=true, async=true)` — `prompt` is the one-line playbook call from Step 3 (`/<playbook> [args] --run <task_id>`) → the receipt carries the `execution_id`. Append an entry to the run ledger `fleet/.orchestrate-runs.yaml`:
